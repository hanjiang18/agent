...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp){
    ...
    //         vector<float> res=GetValue(mImGray);
    //         if (res[1]<1){
    //             if (res[0] > 0){
 ...
 vector<float> Tracking::GetValue(const cv::Mat &picture){
    float sum = 0;
    float avg = 0;
    cv::Scalar scalar;
    int ls[256];
...
bool Tracking::TrackWithMotionModel()
{
    ...
    //cout<<"pose optimization2"<<endl;
    Optimizer::PoseOptimizationClient(*mCurrentFrame);
    //cout<<"end optimization2"<<endl;
...
bool Tracking::TrackLocalMap()
{
    ...
    // Optimize Pose
    //cout<<"pose optimization3"<<endl;
    Optimizer::PoseOptimizationClient(*mCurrentFrame);
    //cout<<"end optimization3"<<endl;
    mnMatchesInliers = 0;
...
cv::Mat Tracking::make_pad(const cv::Mat& one_image, const int pad_H, const int pad_W){
        cv::Mat padded_image;
